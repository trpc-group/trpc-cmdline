...
int {{ $svc.Name }}{{ $method.Name }}(const std::shared_ptr<::{{ $pkgNamespace }}::{{ $svc.Name }}ServiceProxy>& proxy) {
  ...
  ::{{ $rpcRspType }} rsp;
  ::trpc::Status status = proxy->{{ $method.Name }}(client_ctx, req, &rsp);
  if (!status.OK()) {
...
int Run() {
  ...
  return {{ $svc0.Name }}{{ $method.Name }}(proxy);
  {{- else }}
  int ret = 0;
  {{ range $idx, $svc := .Services }}
  {{- $idx_suffix := $idx -}}{{- if (eq $idx 0) -}}{{- $idx_suffix = "" -}}{{- end }}
  ...
  {{ range $method := .RPC -}}
  ret = {{ $svc.Name }}{{ $method.Name }}(proxy{{ $idx_suffix }});
  if (ret < 0) return ret;
  {{ end -}}
  {{ end }}
  return ret;
